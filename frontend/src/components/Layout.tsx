    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row transition-colors duration-200">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-primary text-white sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg">LM Portal</span>
        </div>
      ...
      <main className="flex-1 flex flex-col min-h-0 md:min-h-screen overflow-hidden">
        {/* Desktop Header */}
        <header className="hidden md:flex items-center justify-end px-8 py-4 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30">
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{user?.name || 'Inspector'}</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{user?.role || 'Inspector'} {user?.officerId}</p>
            </div>
