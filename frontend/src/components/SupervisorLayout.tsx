...
export function SupervisorLayout() {
  ...
  const navItems = [
    { name: 'Overview', path: '/supervisor', icon: LayoutDashboard },
    { name: 'Inspectors', path: '/supervisor/inspectors', icon: Users },
    { name: 'Case Review', path: '/supervisor/review', icon: ClipboardCheck },
    { name: 'Enforcements', path: '/supervisor/enforcements', icon: Scale },
    { name: 'Registrations', path: '/supervisor/registrations', icon: Building2 },
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col md:flex-row transition-colors duration-200">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-primary text-white sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-lg">LM Supervisor</span>
        </div>
        ...
        <header className="hidden md:flex items-center justify-between px-8 py-4 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-0 z-30">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-sm font-medium text-slate-900 dark:text-slate-100">{user?.name || 'Controller'}</p>
              <p className="text-xs text-slate-500 dark:text-slate-400">{user?.role || 'Supervisor'}</p>
            </div>
